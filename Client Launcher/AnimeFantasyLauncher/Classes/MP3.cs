...
{
    public class MP3
    {   // http://www.codeproject.com/Articles/14709/Playing-MP-s-using-MCI
        ...
        public static List<MP3> currentlyActive = new List<MP3>();
        public static List<MP3> lastFiveActive = new List<MP3>();
        ...
        private List<MP3> cleanUpActive()
        {
            List<MP3> cachedList = new List<MP3>(currentlyActive);
            cachedList = (from c in cachedList where c.AudioLength == c.CurrentPosition select c).ToList();

            foreach (MP3 eachSound in cachedList)
            {
                eachSound.Stop();
                eachSound.Close();
            }
        ...
        {
            foreach (MP3 eachSound in currentlyActive)
            {
                eachSound.Pause(true);
            }
        ...
        private void stackLastFive(MP3 latest)
        {
